...
MONITOR_FAULT_EP_CAP_IDX = 2
MONITOR_REPLY_CAP_IDX = 3
...
def generate_capdl(system: SystemDescription, search_paths: List[Path], kernel_config: KernelConfig, monitor_path: Path) -> capdl.Spec:
    ...
    monitor_cspace[MONITOR_FAULT_EP_CAP_IDX] = capdl.Cap(monitor_fault_ep, read=True, write=True)

    monitor_reply_object = capdl.RTReply("monitor_reply_object")
    ...
    monitor_scheduling_context = capdl.SC("monitor_scheduling_context", period=100, budget=100)
    monitor_tcb["sc_slot"] = capdl.Cap(monitor_scheduling_context, read=True, write=True)
    cdl_spec.add_object(monitor_scheduling_context)
    # @ivanv: shouldn't we be doing something with this scheduling control?
    ...
    for i, pd in enumerate(system.protection_domains):
        ...
        scheduling_context = capdl.SC(f"scheduling_context_{pd.name}", period=pd.period, budget=pd.budget, size_bits=8)
        tcb["sc_slot"] = capdl.Cap(scheduling_context, read=True, write=True)
        cdl_spec.add_object(scheduling_context)
        # scheduling_control = capdl.SchedControl(f"scheduling_control_{pd.name}")
        # cdl_spec.add_object(scheduling_control)
    ...
    for cc in system.channels:
        ...
        if pd_b.pp:
            ...
            pd_a_cspace[pp_ch_cap] = capdl.Cap(pd_b_endpoint, read=True, write=True, grant=True, grantreply=True, badge=pp_ch_badge)

        if pd_a.pp:
            pp_ch_cap = BASE_OUTPUT_ENDPOINT_CAP + cc.id_b
            pp_ch_badge = (1 << 63) | cc.id_a
            pd_b_cspace[pp_ch_cap] = capdl.Cap(pd_a_endpoint, read=True, write=True, grant=True, grantreply=True, badge=pp_ch_badge)
...
def build_system(
    ...
    if is_capdl:
        # @ivanv: understand this change
        initial_task_phys_regions_elf = phys_mem_regions_from_elf(monitor_elf, kernel_config.minimum_page_size)
    ...
    if is_capdl:
        # @ivanv: understand this change
        initial_task_virt_regions_elf = virt_mem_regions_from_elf(monitor_elf, kernel_config.minimum_page_size)
...
def main() -> int:
    ...
    parser.add_argument("--capdl", action="store_true")
    ...
    normal_monitor_elf_path = elf_path / "monitor.elf"
    capdl_monitor_elf_path = elf_path / "capdl-monitor.elf"
    capdl_initialiser_elf_path = elf_path / "capdl-initialiser.elf"

    monitor_elf_path = capdl_monitor_elf_path if args.capdl else normal_monitor_elf_path
    ...
    if not capdl_monitor_elf_path.exists():
        ...
        return 1
    if not capdl_initialiser_elf_path.exists():
        print(f"Error: capDL initialiser ELF '{capdl_initialiser_elf_path}' does not exist")
        return 1
    ...
    if not args.capdl and len(monitor_elf.segments) > 1:
    ...
    if args.capdl:
        ...
        with open(CAPDL_SPEC_PATH, "w") as f:
            f.write('%s' % cdl_spec)
        capdl_add_spec = SDK_DIR / "bin" / "capdl-spec-add"
        parse_capdl_tool = SDK_DIR / "bin" / "parse-capdl"
